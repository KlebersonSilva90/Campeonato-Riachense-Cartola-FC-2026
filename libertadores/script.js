...
function atualizarGrupo() {
  ...
  if (pasta === "mata-mata") {
    ...
    imgClassificacao.alt = `Confrontos da fase ${fase}`;
    ...
    imgClassificacao.alt = `Classificação do ${tituloGrupo.innerText}`;
    imgRodadas.src = `${pasta}/rodadas.png`;
    imgRodadas.alt = `Rodadas do ${tituloGrupo.innerText}`;
...
document.addEventListener("keydown", (event) => {
  if (event.key === "ArrowLeft") trocarGrupo(-1);
  if (event.key === "ArrowRight") trocarGrupo(1);
});
